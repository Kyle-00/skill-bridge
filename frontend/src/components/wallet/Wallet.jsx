...
import {
  FaPlus, FaArrowUp, FaArrowDown, FaExchangeAlt,
  FaClock, FaCopy, FaCheck,
} from 'react-icons/fa';
...
const Wallet = () => {
  ...
  // A transaction is a debit if it represents money leaving the wallet.
  // The backend stores withdrawals as positive numbers, and transfers as
  // negative for the sender and positive for the recipient. We determine
  // the sign for display using transaction_type plus the stored amount.
  const isDebitTransaction = (tx) => {
    if (tx.transaction_type === 'withdrawal') return true;
    if (tx.transaction_type === 'transfer') return tx.amount < 0;
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl sm:text-3xl font-bold text-gold-700 dark:text-gold-300 mb-6">
        My Wallet
      </h1>
      ...
      <div className="glass p-6 rounded-3xl shadow-xl mb-6">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mt-6">
          ...
          <Link
            to="/wallet/withdraw"
            className="border border-gold-600 text-gold-600 px-4 py-2 rounded-lg hover:bg-gold-50 dark:hover:bg-gold-900/30 transition flex items-center gap-2 text-sm"
          >
            <FaArrowUp size={12} /> Withdraw
          </Link>
          <Link
            to="/wallet/transfer"
            className="border border-blue-600 text-blue-600 px-4 py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition flex items-center gap-2 text-sm"
          >
            <FaExchangeAlt size={12} /> Send
          </Link>
          <Link
            to="/wallet/security"
            className="border border-purple-600 text-purple-600 px-4 py-2 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/30 transition flex items-center gap-2 text-sm"
          >
        ...
        {!hasPin && (
          <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg text-sm text-yellow-800 dark:text-yellow-200">
            Set a wallet PIN before you can withdraw or send money.
            <Link to="/wallet/security" className="ml-1 underline font-semibold">
              Set PIN now
            </Link>
          </div>
        )}
      ...
      <h2 className="text-lg sm:text-xl font-semibold text-gold-700 dark:text-gold-300 mb-4">
        Recent Transactions
      </h2>
